/**
 * @module csv
 * CSV export: generate a CSV string from game store data and trigger a browser download.
 */

import type { Period, Player, Generator, GameOptions } from '$shared/game.js';
...
/** Shape expected by `generateCsvFromData`. */
interface CsvData {
	periods: Period[];
 ...
 */
export function generateCsvFromData(
	data: CsvData,
	isAdmin: boolean,
	participantName: string
): string {
	const playerKeys = Object.keys(data.players).filter((k) => isAdmin || participantName === k);
	const genKeys = Object.keys(data.gens).filter(
		(k) => isAdmin || data.gens[k].owner === participantName
	);
