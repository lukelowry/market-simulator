...
<aside class="sticky top-8 flex flex-col gap-4 max-h-[calc(100vh-140px)]">
	...
	<div class="flex flex-col gap-2 overflow-y-auto flex-1 min-h-0">
		{#each allMarkets as market (market.name)}
			<button
				class="group relative block w-full text-left py-3 px-4 pb-2 border-[1.5px] rounded cursor-pointer transition-all duration-100 ease-brand font-body {selectedMarket === market.name ? 'border-maroon border-l-4 bg-maroon-faint' : 'bg-white border-border-light hover:border-border hover:shadow-xs'}"
			...
			>
				...
				<div class="flex gap-2 border-t border-border-light pt-2">
					...
					<span
						class="flex items-center justify-center w-7 h-6 rounded-sm cursor-pointer transition-all duration-100 ease-brand hover:bg-maroon-faint hover:text-maroon {getMarketVisibility(market.name) === 'public' ? 'text-maroon' : 'text-text-muted'}"
						role="button"
						tabindex="-1"
						title={getMarketVisibility(market.name) === 'public' ? 'Public (visible to players)' : 'Unlisted (link-only)'}
						aria-label="Toggle visibility"
						onclick={(e: MouseEvent) => toggleVisibility(market.name, e)}
						onkeydown={(e: KeyboardEvent) => { e.stopPropagation(); if (e.key === 'Enter') toggleVisibility(market.name, e); }}
					>
						{#if getMarketVisibility(market.name) === 'public'}
